...
export function can_play(who, moves, positionMatrix, dice, hit, out, game) {
  ...
  for (let i = 0; i < 24; i++) {
    if (
      i === 0 &&
      game === 2 &&
      positionMatrix[23 * (who - 1)][3] === 14 &&
      positionMatrix[23 * (who - 1)][4] === who
    ) {
      let firstpiece = 0;
      let fevgcount;
      for (
        fevgcount = 12 * (2 - who);
        fevgcount <= 11 + 12 * (2 - who);
        fevgcount++
      ) {
    ...
    if (positionMatrix[i][4] === who) {
      ...
      for (d = 0; d < 2; d++) {
        if (moves[d] < 1 + doubles) {
          ...
          if (
            positionMatrix[i + s * dice[d]][4] === 0 ||
            positionMatrix[i + s * dice[d]][4] === who ||
            (positionMatrix[i + s * dice[d]][4] === o &&
              positionMatrix[i + s * dice[d]][3] +
                positionMatrix[i + s * dice[d]][5] <
                2 &&
              game !== 2)
          ) {
            ret = 1;
