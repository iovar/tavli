Scenario *rate_scenarios(Scenario *root,int game)//,Scenario *best)
{
	Scenario *best;
	Scenario *temp;
	int best_rating =0;
	...
	do{
		...
		for(i=0;i<24;i++)
		{
			...
			if((temp->positionMatrix[i][4]==2))
			{
				...
				int to_be_hit=chance_to_be_hit(temp->positionMatrix,i,temp->specs->hitYou);
				firstyours=i;
				
				int pieces=temp->positionMatrix[i][3];
			...
			else if((temp->positionMatrix[i][4]==2)&&
			(temp->positionMatrix[i][3]+temp->positionMatrix[i][5]>1))
			{
				if(previous_closed)
					closed_points+=15;
		...
		if(game==0)
		{
			if((entr_closed>2)&&(entr_closed<5))
				rating-=entr_closed*hitchance;
			else if(entr_closed>4)
				rating-=1000*hitchance;
			//try to block out
			...
			if(temp->specs->hitYou>0)
			{
				if((retr_open)&&(temp->specs->hitYou-retr_open>0))
					...
					rating+=(temp->specs->hitYou)*100;
			}
			//retr closed
			if((temp->specs->hitYou>0)&&(retr_closed==6))
					rating+=100;
			//retr_open
			rating-=retr_open*20;
		...
		if((game==1)||(game==2))
		{
			...
			if(((temp->positionMatrix[17][3]+temp->positionMatrix[17][5]>1)||(game==2))&&(temp->positionMatrix[17][4]==2))
				rating+=(game==1)?150:100;
			if(((temp->positionMatrix[12][3]+temp->positionMatrix[12][5]>1)||(game==2))&&(temp->positionMatrix[12][4]==2))
				rating+=(game==1)?200:100;
			if(((temp->positionMatrix[11][3]+temp->positionMatrix[11][5]>1)||(game==2))&&(temp->positionMatrix[11][4]==2))
				rating+=(game==1)?250:100;
			if(((temp->positionMatrix[6][3]+temp->positionMatrix[6][5]>1)||(game==2))&&(temp->positionMatrix[6][4]==2))
				rating+=(game==1)?300:100;
			if(((temp->positionMatrix[5][3]+temp->positionMatrix[5][5]>1)||(game==2))&&(temp->positionMatrix[5][4]==2))
		...
		if(game==2)
		{
			...
			if(temp->specs->outOp>0)
				rating+=temp->specs->outOp*10;
			if(free_gather)
			{
				rating+=temp->specs->outOp*10000+retrieval_area_total(2,temp->positionMatrix)*50;
			}
			if((!free_gather)&&(fevga_block))
				rating+=1000000;
