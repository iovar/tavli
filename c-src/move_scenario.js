...
export function move_scenario(props) {
    const { positionMatrix, dice: mdice, moves: mmoves, hitOp: mhitOp, hitYou: mhitYou, outOp: moutOp, outYou: moutYou, root, stage, game } = props;
	let Last;
	if(stage==0)
		Last=null;
	else Last=root;
	...
	else
	{
		...
		for(i=1;i<24;i++)
		{
			...
			for(d=0;d<2;d++)
			{
				if(i-mdice[d]>=0)
				{
                    let dice[2]=[mdice[0],mdice[1]];
                    let moves[2]=[mmoves[0],mmoves[1]];
					let hitOp={...mhitOp};
					let hitYou={...mhitYou};
                    let outOp= {...moutOp };
                    let outYou= {...moutYou };
                    ...
                    let dummyMatrix=Array.from({ length: 24 }, () => Array.from({ length: 6 }));
					if(suppose_move(positionMatrix,dummyMatrix,i,i-dice[d],moves,dice,mhitYou,hitOp,game))
					{
						if(!Last)
						{
